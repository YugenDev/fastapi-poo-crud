...
@router.get("/", response_model=list[GetCategoryID])
def get_categories(db: Session = Depends(get_db)):
    categories = db.query(DBCategory).all()
    return categories

# Método para obtener una categoría por su id
@router.get("/{category_id}", response_model=GetCategoryID)
...
@router.put("/{category_id}", response_model=GetCategoryID)
...
@router.delete("/{category_id}", response_model=GetCategoryID)
